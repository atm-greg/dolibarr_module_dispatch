...
function _fiche(&$PDOdb,&$dispatch) {
	...
	foreach($object->lines as $k=>&$line){
		...
		if(empty($label)) $label = $line->product->label;

		$pListe[$line->id]['label'] = ($k+1).'/ '.$label;
		$pListe[$line->id]['data-prod'] = $line->fk_product;
		$pListeProd[$line->id] = $line->fk_product;
	}
	...
	foreach($dispatch->TDispatchAsset as $k=>&$da) {
		...
		?><tr class="<?php echo $class ?>">
			<?php if(GETPOST('type_object') !== 'ticketsup') echo '<td>'.$pListe[$da->fk_object]['label'].'</td>'; ?>
	...
	?><tr style="background-color: lightblue;">
        ...
        if(GETPOST('type_object') !== 'ticketsup')
            echo '<td>'.$form->combo('', 'TLine[-1][fk_object]', $pListe, '');
            echo '<input type="hidden" name="TLine[-1][fk_product]">';
            echo '</td>';
        ?>
	...
	<script type="text/javascript">
		$(document).ready(function() {
                $( "select[name='TLine[-1][fk_object]']" ).change(function (e) {
                    $("input[name='TLine[-1][fk_product]']").val($("select[name='TLine[-1][fk_object]'] option:selected").attr('data-prod'));
                    console.log($("input[name='TLine[-1][fk_product]']").val());
                });

			    $( "input[name='TLine[-1][serial_number]']" ).autocomplete({
			      source: "<?php echo dol_buildpath('/dispatch/script/interface.php',1) ?>?get=serial_number&prod="+$("input[name='TLine[-1][fk_product]']").val(),
			      minLength: 1,
			      select: function( event, ui ) {

			      }
			    });
            $( "input[name='TLine[-1][serial_number]']" ).change( function(){
                fk_prod = $("input[name='TLine[-1][fk_product]']").val();
                console.log(fk_prod);
                //$( this ).autocomplete("option", "source", "<?php //echo dol_buildpath('/dispatch/script/interface.php',1) ?>?get=serial_number&prod="+fk_prod);
            });
		});
...
function _header($id,$type_object) {
	...
	else if($type_object=='bonderetour') {
